...
-- Drop and recreate with FK separation support
DROP FUNCTION IF EXISTS public.get_table_ddl(text);

CREATE OR REPLACE FUNCTION public.get_table_ddl(_table_name text, _include_fks boolean DEFAULT true)
RETURNS text
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  ddl := 'CREATE TABLE IF NOT EXISTS public.' || quote_ident(_table_name) || ' (' || newline;
...
-- Also create a helper to get ONLY foreign keys for a table
CREATE OR REPLACE FUNCTION public.get_table_fks(_table_name text)
RETURNS text
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.get_table_ddl(text, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_table_ddl(text, boolean) TO anon;
GRANT EXECUTE ON FUNCTION public.get_table_fks(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_table_fks(text) TO anon;
