...
BEGIN
  ...
  ddl := 'CREATE TABLE public.' || quote_ident(_table_name) || ' (' || E'\n';
  ...
  LOOP
    ddl := ddl || 'ALTER TABLE public.' || quote_ident(_table_name) 
      ...
      || '(' || quote_ident(fk.foreign_column_name) || ');' || E'\n';
  END LOOP;

  -- RLS status
  IF EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE schemaname = 'public' AND tablename = _table_name AND rowsecurity = true
  ) THEN
    ddl := ddl || E'\nALTER TABLE public.' || quote_ident(_table_name) || ' ENABLE ROW LEVEL SECURITY;\n';
  END IF;
