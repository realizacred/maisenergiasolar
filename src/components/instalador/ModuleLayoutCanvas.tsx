 ...
 export function ModuleLayoutCanvas({
   ...
   const [tool, setTool] = useState<"select" | "add">("add");
   ...
   const drawCanvas = useCallback(() => {
     ...
     ctx.restore();
   }, [backgroundImage, modules, selectedModuleId, scale]);
   ...
   const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
     ...
     if (clickedModule) {
       ...
       addModule(x - DEFAULT_MODULE_WIDTH / 2, y - DEFAULT_MODULE_HEIGHT / 2);
     } else {
       setSelectedModuleId(null);
     }
   };
   ...
   const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
     ...
     if (module) {
       setDragStart({ x: x - module.x, y: y - module.y });
       setIsDragging(true);
     }
   };
   ...
   const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
     ...
     setModules((prev) =>
       prev.map((m) =>
         m.id === selectedModuleId
           ? { ...m, x: x - dragStart.x, y: y - dragStart.y }
           : m
       )
     );
   };
         ...
         <div className="flex-1 overflow-hidden flex flex-col p-4 space-y-4">
           ...
           <div className="flex-1 overflow-auto bg-muted/30 rounded-lg border">
             <canvas
               ...
               height={500}
               className={cn(
                 "cursor-crosshair block mx-auto",
           ...
           <div className="text-xs text-muted-foreground text-center">
             {tool === "add" 
               ? "Clique no canvas para adicionar módulos" 
               : "Clique e arraste para mover módulos"}
           </div>
         </div>
